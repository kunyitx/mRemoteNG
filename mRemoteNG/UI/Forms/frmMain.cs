...
public partial class FrmMain
{
    ...
    private void FrmMain_Load(object sender, EventArgs e)
    {
        ...
        if (Settings.Default.StartFullScreen) Fullscreen.Value = true;
        ...
        try
        {
            ...
            switch (m.Msg)
            {
                ...
                case NativeMethods.WM_CHANGECBCHAIN:
                    ...
                    if (m.WParam == _fpChainedWindowHandle) // If wParam is the next clipboard viewer then it
                        // is being removed so update pointer to the next
                        // window in the clipboard chain
                        _fpChainedWindowHandle = m.LParam;
                    else //Send to the next window
                        NativeMethods.SendMessage(_fpChainedWindowHandle, m.Msg, m.LParam, m.WParam);
                    break;
            }
        }
