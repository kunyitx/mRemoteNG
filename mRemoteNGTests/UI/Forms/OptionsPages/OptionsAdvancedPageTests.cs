...
public class OptionsAdvancedPageTests : OptionsFormSetupAndTeardown
{
    ...
    public void SelectingAdvancedPageLoadsSettings()
    {
        ...
        var checkboxTester = _optionsForm.FindControl<CheckBox>("chkAutomaticReconnect");
        Assert.That(checkboxTester.Text,
            Is.EqualTo("Automatically try to reconnect when disconnected from server (RDP && ICA only)"));
    }
}